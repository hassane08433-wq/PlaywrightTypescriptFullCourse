...
for (const record of records) {
   // const skill = record;
    ...
    test('Data Driven Testing Using JSON File in playwright : ' + record.Skill1, async ({ page }) => {
        ...
        await page.getByRole('link', { name: record.Skill1 }).click();

    });

    // test('Data Driven Testing Using JSON File  : ' + record.Skill2, async ({ page }) => {
    //     await page.goto(`https://www.youtube.com`);
    ...
    //     await page.getByRole('link', { name: record.Skill2 }).click();
